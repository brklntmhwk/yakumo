...
{ lib, makeWrapper, symlinkJoin, xorg, }:
let
  inherit (builtins) any concatStringsSep hasAttr;
  inherit (lib)
    concatMapStringsSep escapeShellArg escapeShellArgs getExe getName
    makeBinPath mapAttrsToList optional optionalString;
in {
  mkAppWrapper =
    { pkg, name ? null, bin ? null, flags ? [ ], env ? { }, deps ? [ ], }:
    let
      binName = if bin != null then
        bin
        ...
        ${optionalString hasMan ''
          ...
          ${concatMapStringsSep "\n" (p:
            if p ? "man" then
              ...
              ''echo "No man output for ${getName p}"'') ([ pkg ] ++ deps)}
        ''}
      '';
      meta = (pkg.meta or { }) // {
