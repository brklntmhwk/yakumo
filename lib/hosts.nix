...
let
  ...
  inherit (lib) getName mkDefault optionals warn;

  defaultOverlays = [ self.overlays.default ];

  throwNotFoundErr = { target, name, configPath, }:
    let configPathStr = toString configPath;
    in throw ''
  ...
  mkHost = { builder, platformType }:
    name:
    { username, system, overlays ? defaultOverlays, extraModules ? [ ], }:
    let
      hostConfigs = ../hosts/${name};
      userConfigs = ../users/${username}/hosts/${name};
    in if !pathExists hostConfigs then
      throwNotFoundErr {
        inherit name;
        target = "host";
      ...
      builder {
        ...
        specialArgs = {
          inherit inputs name username system;
          flakeRoot = self;
        };
      };
in {
