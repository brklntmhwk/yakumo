...
in {
  options.yakumo.programs.git = {
    ...
    config = mkOption {
      type = let gitini = types.attrsOf (types.attrsOf types.anything);
      in types.either gitini (types.listOf gitini) // {
        merge = loc: defs:
          let
            config = builtins.foldl' (acc:
              { value, ... }@x:
              acc // (if builtins.isList value then {
    ...
    lfs = {
      ...
      enablePureSSHTransfer = lib.mkEnableOption
        "Enable pure SSH transfer in server side by adding git-lfs-transfer to environment.systemPackages";
    };
  ...
  config = mkIf cfg.enable (mkMerge [
    (let
      ...
      inherit (lib) concatMapStringsSep generators getName optional;
      inherit (pkgs) writeText;

      gitConfig = writeText "config"
    ...
    in {
      ...
      yakumo.user.packages = [ gitWrapped ]
