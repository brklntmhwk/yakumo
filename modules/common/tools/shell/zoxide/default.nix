...
let
  inherit (lib)
    ...
    types
    ;
  cfg = config.yakumo.tools.shell.zoxide;
  zshCfg = config.yakumo.tools.shell.zsh;
in
{
  options.yakumo.tools.shell.zoxide = {
  ...
  config = mkIf cfg.enable {
    ...
    yakumo.tools.shell.zsh = mkIf zshCfg.enable (
      let
    ...
    # programs.zoxide = {
    #   enable = true;
    #   enableZshIntegration = mkIf config.yakumo.tools.shell.zsh.enable;
    #   package = cfg.package;
    # };
  };
}
