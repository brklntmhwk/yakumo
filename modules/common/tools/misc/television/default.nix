...
let
  inherit (lib)
    ...
    types
    ;
  cfg = config.yakumo.tools.misc.television;
...
in
{
  options.yakumo.tools.misc.television = {
  ...
  config = mkIf cfg.enable (mkMerge [
    (mkIf (elem "nix-search-tv" cfg.channels) {
      ...
      yakumo.tools.misc.television.channelSettings.nix-search-tv =
        let
          path = getExe pkgs.nix-search-tv;
        in
        {
          metadata = {
            name = "nix-search-tv";
    ...
    (mkIf (elem "files" cfg.channels) {
      yakumo.tools.misc.television.channelSettings.files =
        let
    ...
    (
      let
        ...
        configDir = runCommand "television-config" { } ''
          ...
          ${concatLines (
            mapAttrsToList (name: channelCfg: ''
              ln -s ${tomlFormat.generate "${name}.toml" channelCfg} $out/channels/${name}.toml
            '') cfg.channelSettings
          )}
        '';
