...
let
  inherit (lib)
    attrNames literalExpression mkEnableOption mkIf mkMerge mkOption
  ...
  syntaxHighlightingSubmodule = types.submodule {
    options = {
      highlighters = mkOption {
        ...
        default = [ ];
        example = [ "brackets" "cursor" "main" "pattern" ];
        description = ''
...
in {
  options.yakumo.shell.zsh = {
    ...
    setOptions = mkOption {
      ...
      '';
      example = [ "EXTENDED_HISTORY" "RM_STAR_WAIT" "NO_BEEP" ];
    };
  ...
  config = mkIf cfg.enable (mkMerge [
    ...
    in {
      assertions = [
        {
          assertion = (cfg.abbreviations != { })
            -> (any (p: p.name == "zsh-abbr") cfg.plugins);
          message = pluginMissingMsg "zsh-abbr" "abbreviations";
        }
        {
          assertion = (cfg.syntaxHighlighting.styles != { }
            || cfg.syntaxHighlighting.highlighters != [ ]
            || cfg.syntaxHighlighting.patterns != { })
            -> (any (p: p.name == "zsh-syntax-highlighting") cfg.plugins);
          message =
            pluginMissingMsg "zsh-syntax-highlighting" "syntaxHighlighting";
        }
      ];
    })
    (let
      inherit (builtins) concatStringsSep getAttr map;
      inherit (lib) escapeShellArg getName mapAttrsToList optionalString;
      inherit (pkgs) writeTextDir;
      inherit (murakumo.wrappers) mkAppWrapper;

      optsStr =
        concatStringsSep "\n" (map (opt: "setopt ${opt}") cfg.setOptions);
      keymapStr = optionalString (cfg.defaultKeymap != null)
        bindkeyCommands.${cfg.defaultKeymap};
      ...
      aliases = concatStringsSep "\n" (mapAttrsToList
        (k: v: "alias -- ${escapeShellArg k}=${escapeShellArg v}")
        cfg.shellAliases);
      ...
      abbrCmds = concatStringsSep "\n" (mapAttrsToList
        ...
        cfg.abbreviations);
      highlightersCmd = "ZSH_HIGHLIGHT_HIGHLIGHTERS+=(${
          concatStringsSep " "
          (map escapeShellArg cfg.syntaxHighlighting.highlighters)
        })";
      ...
      patternsCmd = concatStringsSep "\n" (mapAttrsToList (k: v:
        "ZSH_HIGHLIGHT_PATTERNS+=(${escapeShellArg k} ${escapeShellArg v})")
        cfg.syntaxHighlighting.patterns);
