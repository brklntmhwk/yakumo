...
in {
  options.yakumo.shell.starship = {
    ...
    settings = mkOption {
      type = let
        inherit (types) attrsOf bool either int listOf str;
  ...
  config = mkIf cfg.enable (mkMerge [
    {
      assertions = let
        ...
        hasStarshipEnabled = cfg:
          (anyAttrs isEnabled cfg)
          || !(anyAttrs (_: v: v.settings or false) cfg);
      in [{
        assertion = hasStarshipEnabled cfg;
        message =
          "Starship settings cannot be added without itself being enabled anyway";
      }];
    }
    (let
      ...
      starshipWrapped = mkAppWrapper {
        ...
        env = { STARSHIP_CONFIG = starshipToml; };
      };
    in {
