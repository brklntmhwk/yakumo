...
let
  ...
  inherit (lib) elem mkDefault mkEnableOption mkIf mkMerge mkOption types;
  inherit (murakumo.utils) anyAttrs countAttrs;
  cfg = config.yakumo.desktop;
in {
  options.yakumo.desktop = { enable = mkEnableOption "desktop environment"; };
      ...
      in [
        {
          ...
          message =
            "Multiple compositors or window managers cannot be enabled at a time";
        }
        {
          assertion = hasDesktopEnabled cfg;
          message =
            "Desktop's sub-options cannot be enabled without itself being enabled anyway";
        }
      ];
    }
    (mkIf cfg.enable {
      ...
      environment.systemPackages =
        attrValues { inherit (pkgs) libnotify xdg-utils; };
