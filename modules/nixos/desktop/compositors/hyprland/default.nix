...
in {
  options.yakumo.desktop.compositors.hyprland = {
    ...
    settings = mkOption {
      type = let
        valType = types.nullOr (types.oneOf [
          types.bool
          types.float
          types.int
          types.path
          ...
          description =
            "Hyprland configuration values in Nix-representable Hyprconf format.";
        };
      in valType;
      default = { };
      description = ''
  ...
  config = mkIf cfg.enable (let inherit (builtins) attrValues hasAttr;
  in mkMerge [
    {
      assertions = [
        # https://github.com/nix-community/home-manager/commit/1ecfd8e5626b27a9610f468be32cd6a7011f56a0
        {
          ...
          message =
            "Submaps can't be named 'reset'. The name 'reset' is reserved in order to have a way to switch to the default submap; as if 'reset' was its name.";
        }
      ];
      ...
      xdg.portal = {
        extraPortals = attrValues {
          inherit (pkgs) xdg-desktop-portal-gtk xdg-desktop-portal-hyprland;
        };
        config = {
    ...
    (let
      inherit (lib) concatMapAttrsStringSep getName optional optionalString;
      ...
      hyprlandConf = writeText "hyprland.conf"
        (optionalString (cfg.settings != { })
          (toHyprconf { attrs = cfg.settings; })
          + (optionalString (cfg.submaps != { })
